...
!
  implicit none
   ...
   integer,intent(in out)::nf(:,:)
    ...
    do j=1,ubound(nf,2)
       do i=1,ubound(nf,1)
          if(nf(i,j)/=0) then!不为0的节点自由度会从1开始重新计算；为0的节点自由度仍为0
             m=m+1; nf(i,j)=m
          end if
       end do
    end do
...
idof=size(km,1)! DOF of each element=dimension of stiffness matrix of element
     do i=1,idof
        if(g(i)/=0) then!exclude the element whose DOF is 0,why?
           do j=1,idof
              if(g(j)/=0) then
                 !icd=g(j)-g(i)+1
                 !if(icd-1>=0) then
...
n=ubound(kb,1); iw=ubound(kb,2)-1
   do i=1,n
      ...
      kb(i,iw+1)=sqrt(abs(kb(i,iw+1)-x))!我在此加入了abs()函数
      do k=1,iw
         x=.0
         if(i+k<=n) then
           if(k/=iw) then
