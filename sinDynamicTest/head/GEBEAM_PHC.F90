...
!------------整体节点坐标和整体节点编号；------------------------------
!------------单元自由度编号和整体自由度编号；半带宽--------------------
subroutine geometry_get(iel,ell,stroke,coord,num)
...
implicit none
	  ...
	  real,intent(in)::stroke;
	  
	  num=(/iel,iel+1/);
	  if(iel==1)then
		coord(1,1)=0.0; coord(2,1)=stroke; 		
	  else
		coord(1,1)=(iel-2)*ell+stroke; coord(2,1)=(iel-1)*ell+stroke;
	  end if
