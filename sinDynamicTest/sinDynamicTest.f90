...
!
!  FUNCTIONS:
!  sinDynamicTest - Entry point of console application.
!

!****************************************************************************
!
!  PROGRAM: sinDynamicTest
!
...
!****************************************************************************
    ...
    ! Body of sinDynamicTest
    print *, 'Hello World'

    end program sinDynamicTest
...
allocate(tkm(neq,(2*nband+1)),tmm(neq,(2*nband+1)),tgykm(neq,(2*nband+1)),tctkm(neq,(2*nband+1)),&
		...
		ttd(0:neq),ttv(0:neq),tta(0:neq),ttd_0(0:neq),ttv_0(0:neq),tta_0(0:neq),tkm_bck(neq,(2*nband+1)),tmm_bck(neq,(2*nband+1)),&
...
!------------------------------GET SEA STATE----------------------------------------------
	...
	!amp = 1.0; period = 8;! the 5 sea state;
	omego = 2*pi/period;	
!------------------------------GET LOADS--------------------------------------------------
...
timeloop:do while(ts<=100.0) !自适应步长 dt
	...
	!n+1时间步迭代初值预测（依据n时间步的收敛值d0,v0,a0），迭代过程中更新校正，直到收敛
		...
		iterations:do
			iters = iters+1;
			!if(iters>5)then;dt0 =dt0-0.001;cycle timeloop;end if !迭代次数增加时，要减少步长;
			if(iters>5)then;dt0 =dt0/2.0;cycle timeloop;end if !迭代次数增加时，要减少步长,recommend;
			...
			do iel = 1,nels
				...
				if(iel==1)then;
					...
					!正弦运动，注意母船升沉运动是单自由度运动，初始运动方向指向天空
					!ttd(g(1)) = -amp*sin(omego*ts);
					!ttv(g(1)) = -amp*omego*cos(omego*ts);
					!tta(g(1)) = amp*omego*omego*sin(omego*ts);	
					
					!大洋一号实际测量母船升沉数据，频率10Hz.
				...
				eldd(1,:)=ttd(g(1:3));elphai(1,:)=ttd(g(4:6));
				eldd(2,:)=ttd(g(7:9));elphai(2,:)=ttd(g(10:12));
				
				! extract the velocity of an element from the total
				eldxc(1,:)=ttv(g(1:3));eldphai(1,:)=ttv(g(4:6));
				eldxc(2,:)=ttv(g(7:9));eldphai(2,:)=ttv(g(10:12));
				
				! extract the acceleration of an element from the total
				eld2xc(1,:)=tta(g(1:3));eld2phai(1,:)=tta(g(4:6));
				eld2xc(2,:)=tta(g(7:9));eld2phai(2,:)=tta(g(10:12));
				...
				do k=1,nip2
					...
					if(iel==nels)then!集中水下[中继器与ROV的质量]，及[附加质量=附加质量系数*置换水的质量(浮力/g)]，单位KG
						!emm(7,7)=emm(7,7)+3.0/3.0*(2790+3000+1.5*4320);
						emm(7,7)=emm(7,7)+ masscage;!masscage表示中继器的质量及附加水质量的总和（在F. R. Driscoll中不考虑ROV）。
			...
			kiter=alpha1*tmm +alpha4* tgykm - kcr;	!增量系数矩阵,Ted Belytschko书上称为JACOBIAN矩阵
			!基于位移边界条件，修正JACOBIAN矩阵
			kiter(1,:)=0.0;kiter(1,nband+1)=10000;
			kiter(:,nband+1)=kiter(:,nband+1)+1.0;!为防止线性方程组Ax=b中的A出现病态时导致数值奇异,数字不能加太多，否则偏差亦大。
			
			!求W矩阵各项
			temp1=alpha1*(ttd-d0)-alpha2*v0-alpha3*a0; 
			!CALL LINMUL(tmm,temp1,mtemp);
			CALL BANTMUL(tmm,temp1,mtemp);
			delta=loads+rloads-mtemp;
			delta(0)=0.0;
			delta(1) = 0.0;!修正JACOBIAN矩阵
